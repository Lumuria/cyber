...
const PreventionPage = () => {
  ...
  const filteredPrevention = prevention.filter((item) => {
    const titleText = item?.title?.[i18n.language] || '';
    const descriptionText = item?.description?.[i18n.language] || '';
    const matchesSearch =
    ...
    const matchesDifficulty =
      ...
      (item?.difficulty?.[i18n.language] || '') === selectedDifficulty;
        ...
        {filteredPrevention.map((item) => (
          <div key={item.id} className="prevention-card">
            ...
            <div className="card-tags">
              <span className="tag blue">{item?.category?.[i18n.language] || '-'}</span>

              <span className={`tag importance-${item?.importance?.ar || ''}`}>
                {t('preventionPage.importance')}: {item?.importance?.[i18n.language] || '-'}
              </span>

              <span className={`tag difficulty-${item?.difficulty?.ar || ''}`}>
                {t('preventionPage.difficulty')}: {item?.difficulty?.[i18n.language] || '-'}
              </span>
            </div>
            ...
            <div className="tips-box">
              ...
              <ul>
                {(item.tips || []).map((tip, i) => (
                  <li key={i}>- {tip?.[i18n.language] || '-'}</li>
                ))}
              </ul>
            </div>

            <div className="card-footer">
              <Button className="apply-btn">{t('preventionPage.apply')}</Button>
            </div>
          </div>
        ))}
      </div>
